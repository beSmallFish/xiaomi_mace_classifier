...
Java_com_xiaomi_mace_JniMaceUtils_maceMobilenetClassify(
  ...
  /*  __android_log_print(ANDROID_LOG_ERROR,
                        "image_classify",
                        "input_shape_1: %d, input_shape_2:%d, output_shape_1:%d, output_shape_2:%d, input_size:%d, output_size:%d",
                        input_shape[1], input_shape[2], output_shape[1], output_shape[2], input_size, output_size);*/
...
JNIEXPORT jfloatArray JNICALL
Java_com_xiaomi_mace_JniMaceUtils_maceMobilenetClassify2(
        JNIEnv *env, jclass thisObj, jfloatArray input_data, jint width, jint height, jint rgb) {

  MaceContext &mace_context = GetMaceContext();
  //  prepare input and output
    //mace_context.model_name
 /* auto model_info_iter =
          mace_context.model_infos.find(mace_context.model_name);
  if (model_info_iter == mace_context.model_infos.end()) {
    __android_log_print(ANDROID_LOG_ERROR,
    ...
    struct ModelInfo model_info =  {"input", "output", {1, height, width, rgb}, {1,  targetHeight, targetWidth, rgb}};
    mace_context.model_infos.clear();
    mace_context.model_infos.insert( std::pair<std::string,ModelInfo>("wangsu",model_info));
    const std::string &input_name = model_info.input_name;
    const std::string &output_name = model_info.output_name;
    const std::vector<int64_t> &input_shape = model_info.input_shape;
    const std::vector<int64_t> &output_shape = model_info.output_shape;
...
Mat maceByMat(Mat input_mat, jint rgb) {
    ...
    //  load input
    float * input_array = (float *)input_mat.data;
    ...
    auto buffer_in = std::shared_ptr<float>(new float[input_size],
                                            std::default_delete<float[]>());
    std::copy_n(input_array, input_size, buffer_in.get());
    inputs[input_name] = mace::MaceTensor(input_shape, buffer_in);
    ...
    env->SetFloatArrayRegion(jOutputData, 0, output_size,
                             outputs[output_name].data().get());  // copy*/
    Mat output_mat(input_mat.rows*3, input_mat.cols*3, CV_32F,  outputs[output_name].data().get());
